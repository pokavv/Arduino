...
class Transpiler {
    ...
    _transformVarLine(line) {
        ...
        // C++ 객체 생성자 선언: TypeName varName(args); → let varName = new TypeName(args);
        // 예: DHT dht(9, DHT11);  Servo myServo;  LiquidCrystal lcd(rs,en,d4,d5,d6,d7);
        const ctorMatch = trimmed.match(/^([A-Za-z_]\w*)\s+([A-Za-z_]\w*)\s*\(([^)]*)\)\s*;$/);
        if (ctorMatch && !this._isCppType(ctorMatch[1])) {
            const className = ctorMatch[1];
            const varName   = ctorMatch[2];
            const rawArgs   = ctorMatch[3].trim();
            // & 참조 연산자 제거 (예: &Wire → Wire)
        ...
        if (typeMatch) {
            ...
            if (!this._isCppType(typeName)) {
                // 라이브러리 클래스 인스턴스: LiquidCrystal lcd; → let lcd = new LiquidCrystal();
                if (rawVal) {
                    const value = this._cleanInitializer(rawVal.trim(), typeName);
                    return `${indent}let ${varName} = ${value};`;
                }
    ...
    _buildPreamble() {
        return [
            ...
            'function tone(pin, freq, dur)          { return _runtime.tone(pin, freq, dur); }',
            'function noTone(pin)                   { return _runtime.noTone(pin); }',
            'function pulseIn(pin, val, to)         { return _runtime.pulseIn(pin, val, to); }',
            'function shiftOut(dp, cp, bo, val)     { return _runtime.shiftOut(dp, cp, bo, val); }',
            'function shiftIn(dp, cp, bo)           { return _runtime.shiftIn(dp, cp, bo); }',
