...
export class SimResistor extends SimElement {
  static override styles = [
    SimElement.styles,
    css`:host { width: 105px; height: 48px; }`,
  ];
  ...
  // getPinPositions: 리드선은 viewBox y=14 중앙 → host y=14×1.5=21
  ...
  override render() {
    ...
    return html`
      <svg width="105" height="48" viewBox="0 0 70 32" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- 저항값 라벨 (밝고 큼직하게) -->
        <rect x="17" y="22" width="36" height="10" rx="1" fill="#0d0d14"/>
        <text x="35" y="30" font-size="8" fill="#eecc88" font-family="monospace"
          text-anchor="middle" font-weight="bold">${label}</text>

        <!-- 핀 끝 색상 표시 (리드 선 좌우) -->
        <circle cx="3" cy="14" r="2.5" fill="#666666" stroke="#444" stroke-width="0.5"/>
        <circle cx="67" cy="14" r="2.5" fill="#666666" stroke="#444" stroke-width="0.5"/>
      </svg>
    `;
  }
}
