...
export class SimTransistorNpn extends SimElement {
  ...
  /** BASE 핀이 HIGH(1)이면 트랜지스터 ON (전류 흐름 허용 상태) */
  ...
  override render() {
    // ON 상태일 때 몸체 색상 및 글로우 효과를 변경
    const bodyColor0  = this.active ? '#162030' : '#1a1a1a';
    const bodyColor40 = this.active ? '#1e3a55' : '#2a2a2a';
    const bodyColor65 = this.active ? '#224870' : '#383838';
    const bodyColor85 = this.active ? '#2a5080' : '#444';
    const bodyColor100 = this.active ? '#152535' : '#222';
    const domeColor50 = this.active ? '#2a4a6a' : '#3a3a3a';
    const glowOpacity = this.active ? '0.55' : '0';
    // BASE 핀 라벨: ON이면 밝은 하늘색으로 강조
    const baseLabelColor = this.active ? '#44ddff' : '#88aaff';
      ...
      <svg width="45" height="68" viewBox="0 0 12 18"
        ...
        <defs>
          ...
          <linearGradient id="to92-body-${this.compId}" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%"   stop-color="${bodyColor0}"/>
            <stop offset="40%"  stop-color="${bodyColor40}"/>
            <stop offset="65%"  stop-color="${bodyColor65}"/>
            <stop offset="85%"  stop-color="${bodyColor85}"/>
            <stop offset="100%" stop-color="${bodyColor100}"/>
          </linearGradient>
          <!-- 상단 돔 그라디언트 -->
          <linearGradient id="to92-dome-${this.compId}" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%"   stop-color="#1c1c1c"/>
            <stop offset="50%"  stop-color="${domeColor50}"/>
          ...
          <!-- ON 상태 글로우 (파란 빛) -->
          <radialGradient id="to92-glow-${this.compId}" cx="50%" cy="50%" r="60%">
            <stop offset="0%"   stop-color="#44aaff" stop-opacity="${glowOpacity}"/>
        ...
        <!-- ON 상태일 때 외부 파란 글로우 -->
        <ellipse cx="6" cy="7" rx="6" ry="7"
          fill="url(#to92-glow-${this.compId})"/>
        ...
        <!-- Base (중앙, x=6mm) — ON이면 파란 하이라이트 -->
        <rect x="5.65" y="12.5" width="0.7" height="5.5"
          fill="${this.active ? '#55bbff' : 'url(#to92-lead-' + this.compId + ')'}"/>
        <line x1="6.0" y1="12.5" x2="6.0" y2="18"
          stroke="white" stroke-width="0.2" opacity="${this.active ? '0.6' : '0.3'}"/>
        ...
        <rect x="1.0" y="8.5" width="10" height="4.5"
          ...
          stroke="${this.active ? '#335577' : '#111'}" stroke-width="0.25"/>
        ...
        <line x1="1.0" y1="8.5" x2="11.0" y2="8.5"
          stroke="${this.active ? '#446688' : '#555'}" stroke-width="0.2"/>

        <!-- 상단 하이라이트 (반원 꼭대기 광택) -->
        <ellipse cx="6" cy="3.5" rx="2.5" ry="0.8"
          fill="white" opacity="${this.active ? '0.18' : '0.07'}"/>

        <!-- 측면 광택 하이라이트 (우측 돔 부분) -->
        <path d="M 9.5 5.0 Q 10.5 7.0 10.5 9.5"
          stroke="white" stroke-width="0.4" fill="none" opacity="${this.active ? '0.15' : '0.06'}"/>

        <!-- ─── 실크스크린 라벨 (평면 쪽 중앙) ─── -->
        <text x="6.0" y="11.5" font-size="1.4"
          fill="${this.active ? '#aaccee' : '#888888'}"
          font-family="monospace" text-anchor="middle"
          font-weight="bold" letter-spacing="0.0"
          opacity="0.85">2N2222</text>
        <!-- 제조사 로고 자리 (작은 점) / ON이면 파란 빛 점 -->
        <circle cx="6.0" cy="9.7" r="0.3"
          fill="${this.active ? '#66bbff' : '#666'}"
          opacity="${this.active ? '0.9' : '0.6'}"/>
        ...
        <text x="6.0" y="17.3" font-size="1.5" fill="${baseLabelColor}"
